 ...
 **/
public class SolutionTwoTwoInt {
	private static final int LEN = 801;

	static boolean isOddChar(char value) {
	...
	public static void main(String[] args) throws IOException {
		...
		Map<String, Boolean> isPowerOfTwo = new HashMap<>(20000);
		for (int testCase = 0; testCase < testCaseCount; testCase++) {
			...
			for (int i = 0; i < line.length(); i++) {
				...
				for (int j = i + 1; j <= line.length(); j++) {
					...
					Boolean memoizedResult = isPowerOfTwo.get(substring);
					if (memoizedResult != null) {
						...
						if (bigInteger.bitLength() > 800) {
							isPowerOfTwo.put(substring, false);
							break;
						}
						boolean result = bigInteger.bitCount() == 1;
						if (result) {
							powerOfTwoCount++;
						}
						isPowerOfTwo.put(substring, result);
					}
				}
			}
